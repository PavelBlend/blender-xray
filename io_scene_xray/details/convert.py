...
def bpy_data_to_lvl_dets_struct(context, bpy_obj):
    ...
    lvl_dets.meshes_object = utility._get_object(
        bpy_obj, slots.meshes_object, 'Meshes Object'
        )
    utility._validate_object_type(lvl_dets.meshes_object, 'EMPTY', 'Meshes Object')

    lvl_dets.slots_base_object = utility._get_object(
        bpy_obj, slots.slots_base_object, 'Slots Base Object'
        )
    utility._validate_object_type(lvl_dets.slots_base_object, 'MESH', 'Slots Base Object')

    lvl_dets.slots_top_object = utility._get_object(
        bpy_obj, slots.slots_top_object, 'Slots Top Object'
        )
    utility._validate_object_type(lvl_dets.slots_top_object, 'MESH', 'Slots Top Object')

    lvl_dets.lights = utility._get_image(bpy_obj, ligthing.lights_image, 'Lights')

    if context.level_details_format_version == 'builds_1569-cop':
        if ligthing.format != 'builds_1569-cop':
            raise utils.AppError('Object "{0}" has incorrect light format: "Builds 1096-1558". ' \
                'Must be "Builds 1569-CoP"'.format(bpy_obj.name))
        lvl_dets.format_version = 3
        lvl_dets.light_format = '1569-COP'
        lvl_dets.hemi = utility._get_image(bpy_obj, ligthing.hemi_image, 'Hemi')
        lvl_dets.shadows = utility._get_image(bpy_obj, ligthing.shadows_image, 'Shadows')
    else:
        if ligthing.format != 'builds_1096-1558':
            raise utils.AppError('Object "{0}" has incorrect light format: "Builds 1569-CoP". ' \
    ...
    lvl_dets.mesh_0 = utility._get_image(bpy_obj, meshes.mesh_0, 'Mesh 0')
    lvl_dets.mesh_1 = utility._get_image(bpy_obj, meshes.mesh_1, 'Mesh 1')
    lvl_dets.mesh_2 = utility._get_image(bpy_obj, meshes.mesh_2, 'Mesh 2')
    lvl_dets.mesh_3 = utility._get_image(bpy_obj, meshes.mesh_3, 'Mesh 3')
...
def pixel_color_to_density(lvl_dets, pixels, coord_x, coord_y):
    ...
    for corner_index in range(4):
        ...
        density.append(
            int(round(pixels[pixel_index] / format_.DENSITY_DEPTH, 1)) \
            << (4 * corner_index)
            )
    ...
    for corner_index in range(4):
        ...
        light.append(
            int(round(((
                ...
                pixels[pixel_index_b]) / 3) / format_.DENSITY_DEPTH, 1)) \
                << (4 * corner_index))
