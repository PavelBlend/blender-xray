...
def export_bone(
    ...
    writer.put(
        ...
        xray_io.PackedWriter().putf('<H', fmt.CURRENT_BONE_VERSION)
    )
    ...
    writer.put(fmt.Chunks.Bone.SHAPE, xray_io.PackedWriter()
               ...
               .putf('<9f', *xray.shape.box_rot)
               .putf('<3f', *xray.shape.box_trn)
               .putf('<3f', *xray.shape.box_hsz)
               .putf('<3f', *xray.shape.sph_pos)
               .putf('<f', xray.shape.sph_rad)
               .putf('<3f', *xray.shape.cyl_pos)
               .putf('<3f', *xray.shape.cyl_dir)
    ...
    if bpy_arm_obj.data.xray.joint_limits_type == 'XRAY':
        writer.put(fmt.Chunks.Bone.IK_JOINT, xray_io.PackedWriter()
                .putf('<I', int(ik.type))
                .putf('<2f', ik.lim_x_min, ik.lim_x_max)
                .putf('<2f', ik.lim_x_spr, ik.lim_x_dmp)
                .putf('<2f', ik.lim_y_min, ik.lim_y_max)
                .putf('<2f', ik.lim_y_spr, ik.lim_y_dmp)
                .putf('<2f', ik.lim_z_min, ik.lim_z_max)
                .putf('<2f', ik.lim_z_spr, ik.lim_z_dmp)
                .putf('<2f', ik.spring, ik.damping))
    else:
        writer.put(fmt.Chunks.Bone.IK_JOINT, xray_io.PackedWriter()
                .putf('<I', int(ik.type))
                .putf('<2f', pose_bone.ik_min_x, pose_bone.ik_max_x)
                .putf('<2f', ik.lim_x_spr, ik.lim_x_dmp)
                .putf('<2f', pose_bone.ik_min_y, pose_bone.ik_max_y)
                .putf('<2f', ik.lim_y_spr, ik.lim_y_dmp)
                .putf('<2f', pose_bone.ik_min_z, pose_bone.ik_max_z)
                .putf('<2f', ik.lim_z_spr, ik.lim_z_dmp)
                .putf('<2f', ik.spring, ik.damping))
    if xray.ikflags:
        writer.put(
