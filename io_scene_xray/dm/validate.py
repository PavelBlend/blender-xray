...
def validate_export_object(context, bpy_obj, fpath):
    ...
    if len(bpy_obj.data.uv_layers) > 1:
        raise utils.AppError(
            'mesh "{}" has more than one UV-map'.format(mesh_name)
        )
    ...
    if material_count == 0:
        ...
        raise utils.AppError(
            'mesh "{}" has more than one material'.format(mesh_name)
        )
        ...
        if not bpy_material:
            raise utils.AppError(
                'mesh "{}" has empty material slot'.format(mesh_name)
            )
    ...
    if version_utils.IS_28:
        if bpy_material.use_nodes:
            ...
            raise utils.AppError(
                'material "{}" does not use nodes'.format(mat_name)
            )
    else:
        ...
        if bpy_texture.type == 'IMAGE' or \
            ...
            if context.texname_from_path:
                if bpy_texture.type == 'TEX_ENVIRONMENT':
                    log.warn(
                        'material "{}" has incorrect image node type ' \
                        '(Environment Texture)'.format(mat_name),
            ...
            raise utils.AppError(
                'texture "{0}" has an incorrect type: {1}'.format(
                    bpy_texture.name,
                    bpy_texture.type
                )
            )
